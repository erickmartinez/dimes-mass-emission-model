...
\begin{align}
    A_w(t) = \frac{D}{2}\left( \frac{4}{\rho_{\mathrm{rod}}\pi D^2}\right) m_{\mathrm{lost}}(t) 
    = \frac{2}{\rho_{\mathrm{rod}}\pi D} m_{\mathrm{lost}} (t) 
    \label{eq:A_w_t}
\end{align}
...
\begin{align*}
    \nu(t) = \frac{1}{\rho_{\mathrm{rod}}} \left( \frac{\rho_{\mathrm{rod} \pi D}}{2 m_{\mathrm{lost}}(t)}\right) r(t)
    = \frac{\pi D}{2} \frac{r(t)}{m_{\mathrm{lost}}(t)}
\end{align*}
Which can be written \textbf{explicitly} in terms of $I(t)$, and $T_e(t)$ as
\begin{align}
    \nu(t) = \frac{\pi D}{2} \frac{1}{c \int_0^t I(t')T_e(t') dt} c I(t) T_e(t) 
...
\begin{align}
    \nu_k = \frac{\pi D}{2} \frac{I_k T_{e,k}}{S_k} 
...
Define
\begin{align}
    X_i \equiv I_i T_{e,i}, \quad S_k \equiv \sum_{i=0}^k w_i X_i
\end{align}
...
\begin{align*}
    \nu_k = \frac{\pi D}{2}\frac{I_k T_{e,k}}{S_k} = \frac{\pi D}{2}\frac{(V_k/R)T_{e,k}}{\sum_{i}^kw_i(V_i/R)T_{e,i}} =  \frac{\pi D}{2} \frac{V_k T_e{,k}}{\sum_{i}^kw_iV_i T_{e,i}}
\end{align*}
...
Write $\nu_k = \frac{\pi D}{2} X_k S_k^{-1}$ and use the product rule
\begin{align*}
    \frac{\partial \nu_k}{\partial X_j} &= \frac{\pi D}{2}\left[ \frac{\partial X_k}{\partial X_j}\cdot \frac{1}{S_k} + X_k\cdot\frac{\partial \left(S_k\right)^{-1}}{\partial X_j}\right]
\end{align*}
...
\begin{itemize}
    ...
    \item $\dfrac{\partial S_k}{\partial X_j} = w_j$ for $j\leq k$ (since $S_k=\sum_{i=0}^k w_i X_i$ is linear in each $X_i$), so by the chain rule, $\dfrac{\partial \left(S_k\right)^{-1}}{\partial X_j} = - S_k^{-2}\dfrac{\partial S_k}{\partial X_j} = -\dfrac{w_j}{S_k^2}$
\end{itemize}

Putting these together:
\begin{align}
    \frac{\partial \nu_k}{\partial X_j} &= \frac{\pi D}{2}\left[ \frac{\delta_{jk}}{S_k} - \frac{w_j X_k}{S_k^2} \right], \quad j\leq k
...
so in variance form
\begin{align*}
    \frac{\partial v_k}{\partial T_{e,j}} \delta T_{e,j} = \frac{\pi D}{2}\left[ \frac{\delta_{jk}}{S_k} - \frac{w_j X_k}{S_k^2} \right] I_j \delta T_{e,j}, \quad j\leq k
\end{align*}
...
\begin{align}
    \boxed{
        \delta v_k = \left\{\left( \frac{v_k}{D}\right)^2 \delta D^2 + \sum_{j=0}^k\left[ \frac{\pi D}{2} \left( \frac{\delta_{jk}}{S_k} - \frac{w_j X_k}{S_k^2}\right)\right]^2\delta X_j^2\right\}^{1/2}
    }
\end{align}
...
Uncertainty propagation for the DiMES surface recession rate
    ...
    nu_k = (pi * D / 2) * X_k / S_k,  X_i = I_i * Te_i,  S_k = sum_{i=0}^k w_i * X_i
    I_i  = V_i / R   (same R for every sample; I treated as exact -- see below)

Notes
-----
- rho and c cancel analytically and do not appear as inputs.
- R cancels identically out of nu_k (homogeneous degree 0 in the I_i's), so a
  systematic calibration error dR/R contributes nothing and is not modeled.
- Voltage/current uncertainty is taken as zero (machine epsilon), so I_i is
  treated as exact. The only per-sample randomness is in Te_i.
- dX_i therefore reduces to X_i * (dTe_i / Te_i) = I_i * dTe_i.
"""

import numpy as np


def propagate_analytic(D, dD, I, Te, dTe, w):
    """
    Parameters
    ----------
    D, dD : float
        Rod diameter and its 1-sigma uncertainty.
    I : array_like, shape (N,)
        Current samples (I = V/R), treated as exact (dV = 0).
    Te, dTe : array_like, shape (N,)
        Electron temperature samples and their 1-sigma uncertainties.
    w : array_like, shape (N,)
        Quadrature weights (e.g. trapezoidal dt's). Assumed exact.

    Returns
    -------
    nu, dnu : ndarray, shape (N,)
    """
    I, Te, dTe, w = (np.asarray(a, dtype=float) for a in (I, Te, dTe, w))
    N = len(I)

    X = I * Te
    dX = I * dTe  # = X * (dTe/Te), since dI/I = 0
    ...
    for k in range(N):
        ...
        dnu_dXj = (np.pi * D / 2.0) * (kronecker / Sk - w[j] * X[k] / Sk ** 2)
        var_X = np.sum((dnu_dXj * dX[j]) ** 2)
        var_D = (nu[k] / D * dD) ** 2
...
def propagate_montecarlo(D, dD, I, Te, dTe, w, n_draws=20000, seed=0):
    """Monte Carlo cross-check (I fixed/exact, only D and Te resampled)."""
    ...
    X_s = I[None, :] * Te_s
    S_s = np.cumsum(w[None, :] * X_s, axis=1)
    nu_s = (np.pi * D_s[:, None] / 2.0) * X_s / S_s

    return nu_s.mean(axis=0), nu_s.std(axis=0)


if __name__ == "__main__":
    N = 50
    t = np.linspace(0, 1, N)
    I = 5.0 + 3.0 * np.sin(np.pi * t)   # placeholder current trace (exact)
    Te = 20.0 + 10.0 * t
    dTe = 0.10 * Te
    w = np.gradient(t)
    D, dD = 0.6, 0.02

    nu, dnu = propagate_analytic(D, dD, I, Te, dTe, w)
    nu_mc, dnu_mc = propagate_montecarlo(D, dD, I, Te, dTe, w)
